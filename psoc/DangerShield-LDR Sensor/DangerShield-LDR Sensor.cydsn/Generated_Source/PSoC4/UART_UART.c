    ...
    *******************************************************************************/
    void UART_UartInit(void)
    {
        ...
        UART_INTR_TX_MASK_REG     = UART_UART_DEFAULT_INTR_TX_MASK;
            ...
            #if(UART_UART_INTERRUPT_DISABLE)
                UART_SCB_IRQ_Disable();
            #endif /* (UART_UART_INTERRUPT_DISABLE) */
...
{
    #if(UART_CHECK_UART_WAKE_ENABLE)
    {
        UART_backup.enableState = 0u; /* Keep SCB enabled */
